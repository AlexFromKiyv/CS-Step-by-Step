...
Типи запит — це колекції DbSet<T>(наприклад DbSet<Car>), які використовуються для відображення представлень(views), оператора SQL або таблиць без первинного ключа. Типи запитів додаються до похідного DbContext за допомогою властивостей DbSet<T> і налаштовуються як безключові.
