...
|Attach() AttachRange()|Починає відстежувати сутність/сутності. Сутності з числовим первинними ключами, визначеними як ідентичність, і значенням, що дорівнює нулю, відстежуються як Додані. Усі інші відстежуються як незмінені. Також доступні асинхронні версії.|
...
Запити — це колекції DbSet\<T\>(наприклад DbSet<Car>), які використовуються для відображення представлень(views), оператора SQL або таблиць без первинного ключа. Типи запитів додаються до похідного DbContext за допомогою властивостей DbSet\<T\> і налаштовуються як безключові.
