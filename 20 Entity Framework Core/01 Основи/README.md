...
Коли ви використовуєте ADO.NET, ви завжди повинні пам’ятати про фізичну структуру внутрішньої бази даних. Ви повинні знати схему кожної таблиці даних, створювати потенційно складні SQL-запити для взаємодії з таблицями даних, відстежувати зміни в отриманих (або доданих) даних тощо. Це може змусити вас створити досить багатослівний код C#.
...
dotnet add package Microsoft.EntityFrameworkCore -v X.0.13
dotnet add package Microsoft.EntityFrameworkCore.Design -v X.0.13
dotnet add package Microsoft.EntityFrameworkCore.SqlServer -v X.0.13
...
|DbSet\<T\>|Насправді не є членом DbContext, але властивості цього типу додано до настроюваного похідного класу DbContext. Властивості мають тип DbSet\<T\> і використовуються для запиту та збереження екземплярів сутностей програми. Запити LINQ щодо властивостей DbSet<T> перекладаються на запити SQL.|
...
Коли стратегія виконання активна (про це йдеться в наступній главі в розділі «Відмовостійкість з’єднання»), перш ніж створювати явну транзакцію, ви повинні отримати посилання на поточну стратегію виконання, яка використовується. Потім викличте метод Execute() у стратегії, щоб створити явну транзакцію.
...
Для кожного типу сутності T (наприклад Car) у вашій об’єктній моделі ви додаєте властивість типу DbSet\<T\> (DbSet<Car> Cars { get; set; }) до похідного класу DbContext. Клас DbSet\<T\> — це спеціалізована властивість колекції, яка використовується для взаємодії з постачальником бази даних для читання, додавання, оновлення чи видалення записів у базі даних. Кожен DbSet\<T\> надає низку основних служб для взаємодії з базою даних, включаючи переклад запитів LINQ, що виконуються щодо властивості DbSet<T>, у запити до бази даних постачальником бази даних.

Загальні члени та методи розширення DbSet\<T\>

|Члени та методи розширення|Опис|
|--------------------------|----|
|Add()/AddRange()|Починає відстежувати сутність/сутності в стані Added. Елемент(и) буде додано під час виклику SaveChanges(). Також доступні асинхронні версії.|
...
|Update() UpdateRange()|Починає відстежувати сутність/сутності у Modified стані. Елемент(и) буде оновлено під час виклику SaveChanges. Також доступні асинхронні версії.|
|Remove() RemoveRange()|Починає відстежувати сутність/сутності у стані  Deleted. Елемент(и) буде видалено під час виклику SaveChanges(). Також доступні асинхронні версії.|
|Attach() AttachRange()|Починає відстежувати сутність/сутності. Сутності з числовим первинними ключами, визначеними як ідентичність, і значенням, що дорівнює нулю, відстежуються як Added. Усі інші відстежуються як Unchanged. Також доступні асинхронні версії.|
...
Є дві події, які можуть бути викликані у ChangeTracker. Перша — StateChanged, а друга — Tracked. Подія StateChanged запускається, коли змінюється стан об’єкта. Вона не спрацьовує під час першого відстеження сутності. Подія Tracked спрацьовує, коли сутність починає відстежуватися, або через програмне додавання до екземпляра DbSet<T>, або коли повертається із запиту.
...
У EF Core доступні дві схеми співставлення класів у таблиці: таблиця за ієрархією (table-per-hierarchy TPH) і таблиця за типом (table-per-type TPT). Відображення TPH є типовим і відображає ієрархію успадкування в одній таблиці. TPT відображає кожен клас в ієрархії на окрему таблицю. 
...
Створемо проект AutoLot.TPH та додамо до нього пакети. 
    ...
    dotnet add package Microsoft.EntityFrameworkCore -v X.X.X
    dotnet add package Microsoft.EntityFrameworkCore.Design -v X.X.X
    dotnet add package Microsoft.EntityFrameworkCore.SqlServer -v X.X.X
...
```
Зверніть увагу на властивість типу DbSet<Car> у класі ApplicationDbContext. Це інформує EF Core, що клас Car зіставляється з таблицею Cars у базі даних. Також зауважте, що для класу BaseEntity немає властивості DbSet<T>. Це пояснюється тим, що в схемі TPH вся ієрархія стає єдиною таблицею. Властивості таблиць вище ланцюга успадкування згортаються в таблицю за допомогою властивості DbSet<T>. Це показано наступним SQL:
...
Створимо нову міграцію яка створює базу даних. Виконайте в каталозі проекту.

```console
dotnet ef migrations add Initial_Add_Makes_Cars 
```
...
Додайте властивість до класу ApplicationDbContext:
...
```console
 ...
 dotnet ef migrations script 20241023150501_Initial_Add_Makes_Cars
...
CREATE TABLE [Radios] (
    [Id] int NOT NULL IDENTITY,
...
dotnet ef migrations script 20241023155227_Add_Radios
...
...
| Первинний ключ (Primary key)| Властивості з назвою Id або EntityTypeNameId буде налаштовано як первинний ключ. Ключі типу short, int, long або Guid мають значення, які контролюються сховищем даних. Числові значення створюються як стовпці Identity (SQL Server).|
| Зв’язки (Relationships)| Зв’язки між таблицями створюються, коли між двома класами сутностей є властивості навігації.|
| Зовнішній ключ (Foreign key| Властивості під назвою OtherClassNameId є зовнішніми ключами для властивостей навігації типу <OtherClassName>.|
...
За домовленістю загальнодоступні read-write властивості відображаються на однойменні стовпці. Тип даних відповідає еквіваленту сховища даних типу даних CLR властивості. Для властивостей, які не допускають значення null, у сховищі даних встановлено значення NOT NULL, а для властивостей, що допускають значення null (включаючи типи посилань із можливістю значення NULL, представлені в C#), встановлено значення NULL.
...
dotnet ef database update
```
Ця операція створюють знову стовці необхідного нам типу. Це можна подивитись виконавши команду:
...
dotnet ef database update 
dotnet ef migrations list --no-build
dotnet ef migrations script _ChangeCarAddAnotations --no-transactions
```
...
Цей код показує попередній приклад Car з Fluent API, еквівалентним використаним анотаціям даних що до назви таблиці.
...
```cs
        ...
        modelBuilder.Entity<Make>().ToTable(
            t => t.HasCheckConstraint("CH_Name", "[Name]<>'Lemon'"));
...
```
Можете скасувати міграцію для перевірочного обмеження та видалити міграцію, оскільки далі воно не використовується. Його було додано в цей розділ з метою демонстрації.
...
## Домовленості, анотації та Fluent API, боже!
...
Якшо ви перевірите чи потрібни зміни в БД ви побачите шо ні. 
...
Типи запитів — це колекції DbSet\<T\>(наприклад DbSet<Car>), які використовуються для відображення представлень(views), оператора SQL або таблиць без первинного ключа. Типи запитів додаються до похідного DbContext за допомогою властивостей DbSet\<T\> і налаштовуються як безключові.
...
У EF Core є можливість зіставляти один і той самий клас із кількома об’єктами бази даних. Ці об’єкти можуть бути таблицями, представленнями чи функціями. Наприклад клас CarViewModel із попередньої глави можна зіставити з представленням, яке повертає назву марки з даними автомобіля та таблицею Inventory. Потім EF Core надсилатиме запити з подання та надсилатиме оновлення до таблиці.
...
У наступнй главі дуже детально розглядається виконання операцій CRUD.
...
За рахунок використання більшої пам’яті та дещо повільнішого часу виконання запит можна змінити, щоб забезпечити наявність лише одного екземпляра невідстежуваного автомобіля.
...
dotnet-ef глобальний інструмент CLI EF Core містить необхідні команди щоб створювати/видаляти міграції бази даних і працювати з базою даних (оновлювати, видаляти тощо). Перш ніж ви зможете використовувати глобальний інструментарій dotnet-ef, його потрібно встановити за допомогою такої команди:
...
```
Важливо зауважити, що команди CLI не є командами .Net, тому правила екранування скісних риск і лапок не застосовуються.

Аби краще розібратися з роботою CLI EF Core створіть простий консольний проект наприклад MyShop. Відкрийте командний рядок Tools > Command line > Deweloper PowerShell. Додайте пакети
...
| Команда | Значення в використані |
| -------- | --------- |
|add|Створює нову міграцію на основі різниці змін моделі порівняно з попередньою міграції|
...
```
Не забуваємо записати всі зміни.
...
```console
PS D:\..\MyShop> dotnet ef database update
Build started...
Build succeeded.
...
Виконаємо команди в каталозі проекту.

```console
dotnet ef dbcontext scaffold "Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=MyShop" Microsoft.EntityFrameworkCore.SqlServer -d --no-onconfiguring
...
У цьому розділі розпочалося знайомство з Entity Framework Core. У цьому розділі розглянуто основи EF Core, як виконуються запити та відстежуються зміни. Ви дізналися про формування моделі за допомогою домовленостей, анотацій даних та Fluent API. В останньому розділі було розглянуто можливості інтерфейсу командного рядка EF Core та глобальних інструментів.
