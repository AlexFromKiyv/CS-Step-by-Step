...
VALUES (@p0);
',N'@p0 nvarchar(50)',@p0=N'BMW'
```
Формат запиту пов’язаний із процесом пакетування, який використовується EF Core для покращення продуктивності операцій з базою даних. Усі значення, передані в інструкцію SQL, параметризовані, щоб зменшити загрозу сценарних атак. Також зауважте, що нещодавно додана сутність повертає згенеровані базою даних властивості (як можна здогадатись для заповненя властивостей сутності в пам'яті).

## Як побачити запрос на сервері.

Для того аби відслідковувати які sql-запити виконуються на сервері можна встановити SQL Server Menager System і прочитати як користуватися по запиту "XEvent Profiler".
...
OUTPUT INSERTED.[Id], INSERTED.[Display], INSERTED.[TimeStamp]
VALUES (@p0, @p1, @p2, @p3, @p4);
',N'@p0 nvarchar(50),@p1 datetime2(7),@p2 bit,@p3 int,@p4 nvarchar(50)',@p0=N'Blue',@p1='2012-12-01 00:00:00',@p2=1,@p3=1,@p4=N'Bluesmobile'
```
...
Якщо сутність має числову властивість, визначену як первинний ключ, ця властивість (за замовчуванням) зіставляється зі стовпцем Identity у SQL Server. EF Core вважає будь-яку сутність зі значенням за замовчуванням (нуль) для властивості ключа новою, а будь-яку сутність зі значенням, відмінним від за замовчуванням, уже існуючою в базі даних. Якщо ви створюєте нову сутність і встановлюєте для властивості первинного ключа значення, відмінне від нуля, і намагаєтесь додати її до бази даних, EF Core не зможе додати запис, оскільки вставку ідентифікаційної інформації не ввімкнено на стороні сервера.
...
static void AddRowWithSetIdentityInsert()
{
    ...
    string? schema = metadata?.GetSchema();
    string? tableName = metadata?.GetTableName();
    string sql = $"SET IDENTITY_INSERT {schema}.{tableName} ON";
...
OUTPUT INSERTED.[Display], INSERTED.[TimeStamp]
VALUES (@p0, @p1, @p2, @p3, @p4, @p5);
',N'@p0 int,@p1 nvarchar(50),@p2 datetime2(7),@p3 bit,@p4 int,@p5 nvarchar(50)',@p0=27,@p1=N'Blue',@p2='2012-12-01 00:00:00',@p3=1,@p4=1,@p5=N'Bluesmobile'
...
Змінимо властивість класу Make з IEnumerable\<Car\> на ICollection\<Car\> .
...
Аби не виникло помилке в прикладі перевирте властивість класу Car:
...
```
Метод створює кілька сутностей марки та автомобіля та додає їх до бази даних. Сутності Make створюються та додаються до властивості DbSet\<Make\> Makes похідного ApplicationDbContext, а потім викликається метод SaveChanges(). Процес повторюеться  для записів Car, використовуючи властивість DbSet\<Car\> Cars .
...
```cs
static void ClearTable(ApplicationDbContext context, string entityName)
{
...
7 Pink Pinky
```
У прикладі згенерований запит для query1 і query2 ідентичний. Щоб створити оператор OR, ви повинні використовувати ту саму метод Where() з логічним виразом ||. 
...
На додаток до можливості завантажувати пов’язані дані за допомогою властивостей навігації, EF Core автоматично заповнюватиме сутності під час їх завантаження в засіб відстеження змін. Наприклад, припустимо, що всі записи Make завантажено у властивість колекції DbSet\<Make\>. Далі всі записи автомобіля завантажуються в DbSet\<Car\>. Незважаючи на те, що записи були завантажені окремо, вони будуть доступні один одному через властивості навігації.
...
Методи Include() і ThenInclude() (для наступних навігаційних властивостей) використовуються для перегляду навігаційних властивостей у запитах LINQ. Якщо зв’язок з обов'язковою змінною, механізм перекладу LINQ створить внутрішнє об’єднання. 
...
static void EagerLoading_1()
{
    ...
    foreach (var car in cars)
    {
        Console.WriteLine($"{car.Id}\t{car.PetName}\t{car.MakeNavigation.Name}\t{car.Color}");
...
Властивість MakeNavigation є обов’язковим зв’язком, оскільки властивість MakeId сутності Car не допускає значення NULL. Оскільки це обов’язково, таблиця Make об’єднується з таблицею Inventory за допомогою INNER JOIN. Якщо властивість навігації була б необов’язковою (MakeId було визначено як nullable int), об’єднання було б OUTER JOIN.
...
',N'@p1 int,@p0 nvarchar(50),@p2 varbinary(8)',@p1=1,@p0=N'Green',@p2=0x0000000000007537
```
...
static void UsingFromSqlRawInterpolated()
{
    ...
    var car = query.First();
    Console.WriteLine($"{car.Id} {car.PetName} {car.MakeNavigation.Name}");
}
...
Більш свіжа реалізація SQL Server виконає наступний запит

```cs
exec sp_executesql N'SET IMPLICIT_TRANSACTIONS OFF;
SET NOCOUNT ON;
INSERT INTO [dbo].[Inventory] ([Color], [MakeId], [PetName])
OUTPUT INSERTED.[Id], INSERTED.[DateBuilt], INSERTED.[Display], INSERTED.[IsDrivable], INSERTED.[TimeStamp]
VALUES (@p0, @p1, @p2);
...
OUTPUT INSERTED.[Id], INSERTED.[Display], INSERTED.[TimeStamp]
VALUES (@p0, @p1, @p2, @p3, @p4);
',N'@p0 nvarchar(50),@p1 datetime2(7),@p2 bit,@p3 int,@p4 nvarchar(50)',@p0=N'Yellow',@p1='2021-01-01 00:00:00',@p2=1,@p3=1,@p4=N'Herbie'
```
...
static void ThrowConcurrencyException()
{
    ...
    catch (DbUpdateConcurrencyException ex)
    {
        ...
        PropertyValues currentProperty = entry.CurrentValues;
...
Статичний клас EF.Functions було створено як заповнювач для методів CLR, які транслюються у функції бази даних, використовуючи той самий механізм, що й відображення функцій бази даних, описане в попередньому розділі.Основна відмінність полягає в тому, що всі деталі впровадження обробляються постачальниками баз даних.
|Функція|Опис|
|-------|----|
...
|Collate\<TProperty\>()|Визначає зіставлення, яке буде використовуватися в запиті LINQ.|
...
Наприклад, щоб повернути рядкове значення з бази даних замість decimal, почніть з оновлення класу Car, щоб мати рядкову властивість Price:
...
Тіньові властивості — це властивості, які явно не визначені у вашій моделі, але існують завдяки EF Core. Вони корисні, коли в базі даних є дані, які не слід відображати для зіставлених типів сутностей. Значення та стан цих властивостей повністю підтримується інструментом ChangeTracker. Одним із прикладів використання тіньових властивостей є представлення зовнішніх ключів для властивостей навігації, якщо зовнішній ключ не визначено як частину сутності. Іншим прикладом є часові таблиці. 

Тіньові властивості, які не додає EF Core до сутності, можна визначити лише через Fluent API за допомогою методу Property(). Якщо ім’я властивості, передане в метод Property(), збігається з існуючою властивістю сутності (раніше визначеною тіньовою властивістю або явною властивістю), код API Fluent налаштовує наявну властивість. В іншому випадку для сутності створюється тіньова властивість. Щоб додати тінову властивість типу bool? з назвою IsDeleted до сутності Car зі значенням за замовчуванням false, додайте наступний код до методу Configure() класу CarConfiguration.
```cs
    public void Configure(EntityTypeBuilder<Car> builder)
    {
        //...
...
Доступ до тіньових властивостей можна отримати лише через ChangeTracker, тому їх потрібно завантажити з бази даних як відстежувані сутності. Наприклад, наступний код не компілюється:
...
```
Щойно запис додано до засобу ChangeTracker, можна отримати доступ до властивості IsDeleted:
...
Тіньові властивості також можна отримати в запитах LINQ. Наступний код спочатку завантажує всі записи про автомобіль у ChangeTracker, а потім встановлює IsDeleted = false для кожного у якого парне Id.
```cs
...
Додамо в БД схему audits. Для цього додамо нову міграцію.
...
dotnet ef database update
```
Після того, як цю зміну буде перенесено до бази даних, під час перевірки таблиць за допомогою  SSMS ви побачите, що таблиця Inventory позначена як System-Versioned, а коли ви розгорнете вузол, ви побачите audits.InventoryAudit.
...
## Проблеми з FromSqlRaw()/FromSqlInterpolated()
...
Це порушує деякий попередній код у цьому розділі. Щоб виправити це, спочатку поверніться до  UsingFromSqlRawInterpolated() і переконайтесь шо він не працює.
...
Щоб побачити це в дії, подивимось як локальна функція додає  запис в таблиці Inventory за допомогою звичайної взаємодії EF Core:
...
static void ClearSampleDataAndTemporal()
{
    ...
    foreach (var entityName in entities)
    {
        ...
        Console.WriteLine($"All deleted from table {schemaName}.{tableName}.");
        if (entity.IsTemporal())
        {
            ...
            strategy.Execute(() =>
            {
                ...
                trans.Commit();
                Console.WriteLine($"All deleted from table {historySchema}.{historyTable}.");
            });
        }
    }
}
...
```
```
All deleted from table .Drivers.
All deleted from table dbo.Inventory.
All deleted from table audits.InventoryAudit.
All deleted from table .Makes.
```
...
Тепер, коли у вас є міцна основа для роботи з EF Core, у наступному розділі буде створено рівень доступу до даних AutoLot (data access layer).