...
Якщо ви користуєтеся машиною на базі Windows і встановили Visual Studio 2022, у вас також встановлено спеціальний екземпляр SQL Server Express (називається LocalDb), який можна використовувати для всіх наступних прикладах. Якщо ви готові використовувати цю версію, перейдіть до розділу «Встановлення IDE SQL Server».
...
## Відновлення БД в MSSQL extension for Visual Studio Code.

Azure Data Studio стає не актуальним тому всі можливості переходять в MSSQL extension for Visual Studio Code.

Якшо ви корсистуєтесь VSC встановіть розширення MSSQL extension. 

    1.На боковій панелі виберіть SQL Server. 
    
    2.Вибіріть Add Connection. В Connection Dialog заповніть.

    Server name*: (localdb)\mssqllocaldb
    Authentication type*: Windows Authentication
    Database name: AutoLotLocalDb 

    3. Визначення логічних імен файлів: Перед відновленням потрібно знайти логічні імена файлів бази даних у файлі резервної копії (.bak). Виконайте команду RESTORE FILELISTONLY, замінивши шлях фактичним шляхом до файлу резервної копії:

    ```sql
    RESTORE FILELISTONLY FROM DISK = 'D:\Temp\Databases\AutoLotWindows.bak';
    ```

    В області результатів зверніть увагу на значення LogicalName як для файлу даних (зазвичай закінчується на .mdf або подібний), так і для файлу журналу (зазвичай закінчується на .ldf). У нашому випадку це AutoLot та AutoLot_log

    4. Виконайте команду RESTORE: Відкрийте нове вікно запиту та виконайте команду RESTORE DATABASE. Вам потрібно буде вказати:

        Нова назва бази даних :AutoLot.

        Фізичні шляхи для нових файлів .mdf та .ldf у локальній файловій системі сервера.

        Логічне ім'я, визначене на попередньому кроці (наприклад, AutoLot та AutoLot_log) за допомогою параметра MOVE.

        ```sql
        RESTORE DATABASE AutoLot
        FROM DISK = 'D:\Temp\Databases\AutoLotWindows.bak'
        WITH MOVE 'AutoLot' TO 'D:\Temp\Databases\AutoLot.mdf',
        MOVE 'AutoLot_log' TO 'D:\Temp\Databases\AutoLot.ldf',
        REPLACE; -- Use REPLACE if the database 'NewDatabaseName' already exists
        ```

Таким чином ви відновили відновили БД. Цей метод відновленя ви можете отримати за мережевим запитом "MSSQL extension for Visual Studio Restore Database".



# Робота з Базою даних

Стосовно бази даних є два варіанти. Перший варіант відновити існуючу БД з резервонї копії. Другий створити нову базу за допомогою запитів до сервера БД.
...
Якшо ви корсистуєтесь VSC встановіть розширення MSSQL extension. 
    ...
    4. Виконайте команду RESTORE: Відкрийте нове вікно запиту та виконайте команду RESTORE DATABASE. Вам потрібно буде вказати:
        ...
        WITH MOVE 'AutoLot' TO 'D:\Temp\Databases\AutoLot.mdf',
        MOVE 'AutoLot_log' TO 'D:\Temp\Databases\AutoLot.ldf',
        REPLACE; -- Use REPLACE if the database 'AutoLot' already exists
        ```

Таким чином ви відновили відновили БД. Цей метод відновленя ви можете отримати за мережевим запитом "MSSQL extension for Visual Studio Restore Database".

Для того аби виконати backup бази даних можна виконати наступний запит

```sql

BACKUP DATABASE [AutoLot] 
TO DISK = 'D:\Temp\AutoLot.bak'
WITH INIT;
```

# Створення бази даних з нуля.

Створення бази даних з нуля може дати більше розуміння з чого вона складається і як створити свою власну. Можна використати або Visual Studio Code або SQL Server Management Studio. 

## Строрення БД.

Щоб створити базу даних AutoLot, підключіться до сервера бази даних за допомогою Visual Studio Code. Відкрийте новий запит, вибравши  AutoLotLocalDb ➤ New Query (або натиснувши Ctrl+N) і ввівши такий текст команди:
...
Додайте новий файл JSON під назвою appsettings.json. Оскільки БД може бути в контейнері Docker або в окремому екземплярі SQL Sever варіанти підключень можуть бути різними. Ось файл з різними варіантам.
...
Тепер, коли ви краще розумієте роль об’єкта підключення, наступний порядок роботи — перевірити, як надсилати SQL-запити до відповідної бази даних. Тип SqlCommand (який походить від DbCommand) є об'ектним представленням SQL-запиту, імені таблиці або збереженої процедури. Ви вказуєте тип команди за допомогою властивості CommandType, яка може приймати будь-яке значення з enum CommandType , як показано тут:
...
AutoLot.DataAccessLayer\GlobalUsings.cs
```cs
...
Додамо інтерфейс IDisposable до визначення класу. Реалізуемо disposable патерн, викликавши Dispose в об’єкті SqlConnection.
...
В класі операції з данними InventoryDal почнемо з того що ми вже знаемо про об’єкти Command, DataReaders і загальні колекції, щоб отримати записи з таблиці Inventory. Як ви бачили раніше, об’єкт DataReader постачальника даних дозволяє вибирати записи за допомогою механізму лише для читання та проходу в перед за допомогою методу Read().
...
```
Метод GetAllInventory() повертає List<CarViewModel> для представлення всіх даних у таблиці Inventory. У цьому прикладі властивість CommandBehavior на DataReader налаштовано на автоматичне закриття з’єднання, коли закривається зчитувач.
...
Кращий метод використовує як параметр тип Car для створення строго типізованого методу, гарантуючи, що всі властивості передаються в метод у правильному порядку.
...
Транзакція - це набір операцій з базою даних, які успішно або невдало виконуються як сукупна одиниця. Якщо одна з операцій виявляється невдалою, усі інші операції відкочуються, ніби нічого й не було. Транзакції дуже важливі для забезпечення безпеки, дійсності та узгодженості даних таблиць.
...
DataSet частіше за все використовували для відображеня даних.
...
```
При створенні об'єкта SqlCommandBuilder в ньому створюються всі необхідні команди для внесеня змін в БД. Для цього просто в конструктор передаеться об'єкт адаптера. Всі зміни в БД перносяться з пам'яті при виклику методу адаптера Update. Слід зауважити шо це працює тільки для запитів з однією таблицею. Тобто DataSet може бути корисним для показу даних з кількох табоиць але для змін тільки в одній таблиці. Ще один приклад простого використання
...
Спеціальний зчитувач даних краще зробити узагальненим типом який є моделью даних шо ви хочете імпортувати. Почніть із створення нової папки в проекті AutoLot.DataAccessLayer під назвою BulkImport. У папці створіть новий клас інтерфейсу під назвою IMyDataReader.cs, який реалізує IDataReader.
...
```cs
    public MyDataReader(List<T> records, SqlConnection connection, string schema, string tableName)
    {
        ...
        for (int i = 0; i < schemaTable?.Rows.Count; i++)
        {
            ...
            _nameDictionary.Add(i, columnName);
        }
    }
```
...
Додамо в GlobalUsing.cs

```cs
global using AutoLot.DataAccessLayer.BulkImport;
```
...
Далі ви дізналися, як захистити код маніпулювання даними за допомогою транзакцій, і завершили розділ розглядом використання класу SqlBulkCopy для завантаження великих обсягів даних у SQL Server за допомогою ADO.NET.