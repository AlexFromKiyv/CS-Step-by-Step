...
Ключове слово async у C# використовується для визначення того, що метод, лямбда-вираз або анонімний метод повинні викликатися автоматично асинхронно. Просто позначивши метод модифікатором async, середовище виконання .NET створить новий потік виконання для обробки поточного завдання. Крім того, коли ви викликаєте асинхронний метод, ключове слово await автоматично призупиняє поточний потік від будь-якої подальшої активності, доки завдання не буде завершено, залишаючи викликаючий потік вільним для продовження.
...
DoWorkAsync();
```
Після такого виклику ви нічого не побачите, бо при завершені основного потоку зупиняється вториний потік. Середовище попереджає про це.
...
Реалізація DoWorkAsync() тепер безпосередньо повертає об'єкт Task\<string\>, який є значенням, що повертається функцією Task.Run(). Метод Run() приймає делегат Func<> або Action<>, і, як ви знаєте, ви можете спростити собі життя, використовуючи лямбда-вираз. Тобто тут діють правила на створення Task\<T\>. По суті, ваша нова версія виклику DoWorkAsync() фактично говорить наступне:
    ...
    Коли ви мене викличете, я запущу нове завдання. Це завдання переведе вториний потік у режим сну на п'ять секунд, а після його завершення поверне мені рядкове значення. Я помістю цей рядок у новий об'єкт Task<string> та поверну його викликаючій стороні.
...
Як ми обговорювали раніше, коли делегат ставиться в чергу для асинхронного виконання, його запуск планується в окремому потоці. Ця деталізація обробляється середовищем виконання .NET. Зазвичай це робиться за допомогою керованого пулу потоків середовища виконання .NET, але це можна змінити за допомогою спеціальної реалізації.
...
## Роль ConfigureAwait (це може буде заcтаріле)
...
```
Початковий блок коду використовує SynchronizationContext, наданий фреймворком (у цьому випадку, середовищем виконання .NET). Це еквівалентно виклику ConfigureAwait(true). Другий приклад ігнорує поточний контекст і планувальник.

Рекомендації команди .NET пропонують під час розробки коду програми (WinForms, WPF тощо) залишати поведінку за замовчуванням. Якщо ви пишете не-програмний код (наприклад, код бібліотеки), використовуйте ConfigureAwait(false). Єдиним винятком є ​​ASP.NET Core (розглянуто, далі). ASP.NET Core не створює власний SynchronizationContext; тому ConfigureAwait(false) не забезпечує переваги при використанні інших фреймворків.
...
```
    ...
    Awaited метод — це просто метод, який повертає Task або Task<T>.
...
### Методи async Task шо нічого не повертають.
...
```
Підсумовуючи, вам слід уникати створення асинхронних методів що повертають void та використовувати асинхронні методи що повертає Task. Чи чекати на них, чи ні, стає бізнес-рішенням, але в будь-якому разі, принаймні ви не призведете до збою вашої програми.
...
Це поводиться подібно до попереднього прикладу виклику асинхронних методів поверненням Task. Будь-які значення, які повертає метод, втрачаються, а винятки проковтуються.
...
Для демонстрації ми використаємо той самий проект WPF, що й раніше. Створемо проект типу WPF Application з назвою PictureHandlerWithAsyncAwait. Встановити пакет System.Drawing.Common. Tools > NuGet Package Manager > Manage NuGet Packages for Solution > В рядку пошуку System.Drawing.Common > Install
...
```
Метод оголошено як async, повертає IAsyncEnumerable<int> та використовує yield return для повернення цілих чисел з послідовності. Щоб викликати цей метод, додайте наступний код:
```cs
