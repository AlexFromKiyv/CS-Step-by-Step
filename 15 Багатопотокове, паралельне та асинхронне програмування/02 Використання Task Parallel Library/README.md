...
Ключове слово async використовується для визначення того, що метод , лямбда-вираз аьо анонімний метод повинні викликатися в асінхронному режимі автоматично. Просто позначивши метод модифікатором async, .NET Core Runtime створить новий потік виконання для вирішення завдання методу. Крім того, коли ви викликаєте асинхронний метод, ключове слово await автоматично призупинить поточний потік від будь-якої подальшої діяльності до завершення завдання, залишаючи потік, що викликає, вільним для продовження.
...
```
```
I star to do long work synchronous! Thread: 1
...
Долі async метод може поверати лише не будь-шо а лише void, Task, Task<T>, ValueTask<T> типи. Замість того щоб повертати конкретне значення, як наприклод рядок, ми повертаем об'єкт Task<T> де T тип значення що повертається. Якшо метод не повертає значення просто повертаеться Task. 

В прикладі коли виконується виклик DoLongWorkAsync відбуваеться запуск нового завдання. Це завдання призведе до того, що викликаючий потік буде сплячим протягом п’яти секунд, і коли це буде виконано, далі повертає значення рядка. Цей рядок поміщаеться в новий об'єкт Task<string> який поверає await.


Оскільки в цому прикладі await викликається в основному потоці призупиняеться основний поток. 

Якшо обгорнути виклик методу в додадковий метод він не буде блокувати головний потік.
```cs
static async Task<string> DoLongWorkAsync()
{
...
Hi girl
I call DoLongWorkAsync in Thread:6

Done with work. Thread: 6
I continue girl. Goobay!

```
Коли потік логіки досягає await, викликаючий потік призупиняється в цьому методі до завершення виклику. Якби це була графічна програма, користувач міг би продовжувати використовувати інтерфейс користувача, поки виконується метод DoWorkAsync().
