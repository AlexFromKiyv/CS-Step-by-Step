...
Попередні версії ASP.NET використовували файл web.config для налаштування служб і програм, а розробники отримували доступ до параметрів конфігурації через клас System.Configuration. Звісно, всі налаштування конфігурації сайту, а не лише налаштування, специфічні для програми, були збережені у файлі web.config, що (потенційно) зробило його складним безладом.
ASP.NET Core використовує нову систему конфігурації .NET на основі JSON, вперше представлену в розділі "Збірки .Net та конфігуруваня проекту". Нагадуємо, що вона базується на простих JSON-файлах, які містять параметри конфігурації. Файлом конфігурації за замовчуванням є файл appsettings.json. Початкова версія файлу appsettings.json (створеного веб-застосунком ASP.NET Core та шаблонами служби API) містить лише конфігураційну інформацію для ведення журналу, а також дозволяє всім хостам (наприклад, https://localhost:xxxx) прив’язуватися до застосунку:
...
Сервіси в контейнері DI можна впроваджувати в конструктори та методи класів, у представлення Razor, а також у сторінки Razor та класи PageModel. Під час впровадження в конструктор контролера або класу PageModel додайте тип, який потрібно ввести в конструктор, ось так:
...
```
Далі нам потрібно створити модель перегляду для зберігання інформації про дилера. У цій папці додайте новий клас з назвою DealerInfo.cs. Оновіть клас до наступного:
...
Метод Configure<>() колекції IServiceCollection зіставляє розділ конфігураційних файлів з певним типом. Цей тип потім можна впровадити в класи та представлення за допомогою шаблону options. У файлах Program.cs для файлів AutoLot.Mvc та AutoLot.Web Program.cs додайте наступний код після рядка, який використовується для налаштування сервісів даних:
...
Обгортка сервісу AutoLot API використовує типізований базовий клієнт та типізовані клієнти, специфічні для сутності, для інкапсуляції всіх викликів до проекту AutoLot.Api. Як проєкти AutoLot.Mvc, так і AutoLot.Web використовуватимуть обгортку сервісу через класи сервісів даних, які були розглянуті раніше в цьому розділі та завершені в кінці цього розділу. Проєкт AutoLot.Api буде завершено в наступному розділі.
...
```
Створіть нову папку з назвою Base в каталозі ApiWrapper проекту AutoLot.Services та додайте клас з назвою ApiServiceWrapperBase. Зробіть клас публічним та абстрактним і реалізуйте інтерфейс IApiServiceWrapperBase.
...
Клас містить чотири методи підтримки, які використовуються публічними методами.
