...
Перегляди сторінок Razor є специфічними для Razor PageModel, починаються з директиви @page та вводяться в код за файлом, ось так для шаблонної сторінки RazorSyntax:
...
## Каталог Shared
Shared каталог у розділі Pages містить часткові перегляди, шаблони відображення та редагування, а також макети, доступні для всіх сторінок Razor.
...
Створіть нову папку з назвою DisplayTemplates у папці Pages\Shared. Додайте новий Razor View з назвою DateTime.cshtml. Очистіть весь згенерований код та коментарі та замініть їх наступним:
...
В перших рядках перегляду RazorSyntax додайте.
...
@dateTime
```
З шаблоном, якщо ви запустите програму та перейдете на сторінку RazorSyntax, ви побачите, що значення dateTime відформатоване як коротка дата.
...
```
Ви також можете встановити властивості ViewData, додавши до властивості PageModel атрибутом [ViewData]. Наведений нижче код дає той самий результат, що й блок view Razor, показаний у попередньому прикладі коду:
...
```
Тепер в шаблоні редагування використовуюця ці данні.
...
```
Для того шоб це працювало треба щоб ця сторінка вже була готова. На даний момент немає сторінки Details.
...
Метод BuildContent() будуть викликається похідними класами для побудови HTML-коду, який відображатиметься, замість тег-хелпера:
...
Далі ми створимо базовий клас, який оброблятиме спільний код на всіх сторінках для Cars. Перш ніж почати, додайте наступний оператор using до файлу GlobalUsings.cs:
...
Далі додайте метод, який приймає екземпляр IDataServiceBase, імена властивостей dataValueField та dataTextField та створює SelectList:
...
Почніть з додавання порожньої Razor Page з назвою Index.cshtml до каталогу Pages\Cars. Перевизначте клас IndexModel. Додайте конструктор, який отримує екземпляри IAppLogging\<IndexModel\> та ICarDataService та пердає в батьківський клас:
...
```
Щоб побачити цей вигляд у дії, запустіть програму та перейдіть за посиланням https://localhost:5021/Cars/ або https://localhost:5021/Cars/Index, щоб переглянути повний список Car. Щоб переглянути список автомобілів BMW, перейдіть за посиланням https://localhost:5021/Cars/5/BMW (або https://localhost:5021/Cars/Index/5/BMW).
...
```
Тепер ми можемо протестувати цей метод на сторінці Details.
...
```
Цей метод буде протестован в розділі Area де зберігається сутьність Make яка немає залежності від інших.
...
```
Метод обробника HTTP get заповнює властивість LookupValues за допомогою методу базового класу. Оскільки повертаного значення немає, представлення відтворюється після завершення методу:
...
```
Прихований рядок використовується аби прошла валідація поля TimeStamp.
...
```
Метод обробника HTTP-popst використовує базовий метод DeleteOneAsync(), а потім повертає IActionResult з базового методу:
...
Нарешті, відкрийте фрагмент _Menu.cshtml змінить наявну ромітку для сторінки Cars та інші:
...
Areas в Razor Pages дещо відрізняються від Areas у застосунках на основі MVC. Оскільки сторінки Razor маршрутизуються на основі структури каталогів, немає жодної додаткової конфігурації маршрутизації. Єдине правило полягає в тому, що сторінки повинні знаходитися в каталозі Areas\\[AreaName]\\Pages. 
Щоб додати область в AutoLot.Web, спочатку додайте каталог з назвою Areas у кореневому каталозі проекту. Далі додайте каталог з назвою Admin, потім новий каталог з назвою Pages. Нарешті, додайте новий каталог з назвою Makes.

## Маршрутизація Areas в Razor Pages

Під час переходу до сторінок Razor в області ім'я каталогу Areas пропускається. Наприклад, сторінку Index у каталозі Areas\Admin\Makes\Pages можна знайти за маршрутом /Admin/Makes (або Admin/Makes/Index).

## _ViewImports та _ViewStart
У Razor Pages файли _ViewImports.cshtml та _ViewStart.cshtml застосовуються до всіх представлень на одному рівні каталогу та нижче. Перемістіть файли _ViewImports.cshtml та _ViewStart.cshtml до кореневого каталогу проекту, щоб вони застосовувалися до всього проекту.

# Makes Razor Pages

Сторінки для підтримки операцій CRUD для адміністративної області Make відповідають тому ж шаблону, що й сторінки Cars. Вони будуть перелічені тут з мінімальним обговоренням.

## Шаблон відображення Make

Додайте новий каталог з назвою DisplayTemplates у каталозі Makes в області адміністрування. Додайте нове Razor View – Empty з назвою Make.cshtml у новому каталозі. Оновіть вміст до наступного:
...
## Шаблон редагування Make

Додайте новий каталог під назвою EditorTemplates у каталозі Makes в області адміністрування. Додайте нове подання Razor – Empty з назвою Make.cshtml у новому каталозі. Оновіть вміст до наступного:
...
## Код Razor Pages Index для Makes 

На сторінці індексу буде показано список записів Make та посилання на інші сторінки CRUD. Додайте порожню Razor Page з назвою Index.cshtml до каталогу Pages\Makes та оновіть вміст до наступного:
...
## Перегляд Razor Page Index для Makes

Оскільки клас Make такий малий, частковий вираз не використовується для відображення списку записів. Оновіть файл Index.cshtml до наступного:
...
<table class="table">
    <thead>
        <tr>
            <th>@Html.DisplayNameFor(model => ((List<Make>)model.MakeRecords)[0].Id)</th>
...
```
Щоб побачити це в дії, запустіть програму та перейдіть за посиланням https://localhost:5021/Admin/Makes, щоб переглянути повний список записів.

## Додавання елемента меню

Оновіть частковий код _Menu.cshtml, щоб додати елемент меню для області адміністрування, додавши наступне: 
...
```
Під час створення посилання на область потрібен тег-хелпер asp-area, а повний шлях до сторінки має бути розміщений у тег-хелпері asp-page.

## Код Razor Page Details для Makes

Сторінка Details використовується для відображення одного запису під час виклику із запитом HTTP get. Додайте пусту Razor Page з назвою Details.cshtml до каталогу Pages\Makes та оновіть вміст до наступного:
...
```
## Перегляд Razor Page Details для Makes

Оновіть перегляд сторінки Details.cshtml Razor до наступного:
...
## Код Razor Page Create для Makes

Додайте порожню Razor Page під назвою Create.cshtml до каталогу Pages\Makes та оновіть вміст до наступного:
...
## Перегляд Razor Page Create для Makes

Оновіть перегляд Razor Page Create.cshtml  до наступного:
...
## Код Razor Page Edit для Makes

В цьому класі вперше використовується метод базового класу SaveOneAsync() який не потребує списку для виботу. Додайте порожню Razor Page під назвою Edit.cshtml до каталогу Pages\Makes та оновіть вміст до наступного:
...
public class EditModel : BasePageModel<Make,EditModel>
{
    public EditModel(
        ...
        IMakeDataService makeService) : base(appLogging, makeService, "Edit")
    {
    }
...
## Перегляд Razor Page Edit для Makes

Оновіть перегляд Razor Page Edit.cshtml до наступного:
...
## Код Razor Page Delete для Makes

Додайте порожню Razor Page під назвою Delete.cshtml до каталогу Pages\Makes та оновіть вміст до наступного:
...
```
## Перегляд Razor Page Delete для Makes

Оновіть перегляд Razor Page Delete.cshtml до наступного:
...
# Налаштовувані атрибути перевірки

Налаштовувані атрибути перевірки, створені в попередній главі, працюють як з MVC, так і з Razor Page застосунками. Щоб продемонструвати це, створіть нову пусту сторінку Razor під назвою Validation.cshtml у головному каталозі Pages. Оновіть код ValidationModel до наступного:
...
```
Метод обробника HTTP get створює новий екземпляр AddToCartViewModel та призначає його BindProperty. Сторінка автоматично відтворюється після завершення роботи методу. 
Метод обробника HTTP post перевіряє наявність помилок ModelState, і якщо помилка є, повертає некоректні дані до представлення. Якщо перевірка пройшла успішно, він перенаправляє на обробник сторінки HTTP get, дотримуючись шаблону post-redirect-get (PRG).
Тут показано сторінку перевірки, яка є тим самим кодом, що й версія MVC, з єдиною різницею – використання тег-хелпер asp-page замість asp-action:
...
```
Далі додайте пункт меню для переходу до Validation :
...
```
Серверна перевірка вбудована в атрибути, тому ви можете експериментувати зі сторінкою та бачити, як помилка перевірки повертається на сторінку та відображається за допомогою тег-хелперів валідації.
Далі скопіюйте скрипти перевірки з попереднього розділу або створіть їх звідси. Щоб створити їх, створіть новий каталог з назвою validations в каталозі wwwroot\js. Створіть новий файл JavaScript з назвою errorFormatting.js та оновіть його вміст до наступного:
...
```
Далі додайте файл JavaScript з назвою validators.js та оновіть його вміст відповідно до цього. Зверніть увагу на оновлення префікса Entity_ у версії MVC. Це пов'язано з назвою Entity властивості BindProperty:
...
```
Оновіть виклик AddWebOptimizer() в операторах верхнього рівня Program.cs, щоб об'єднати нові файли, коли вони не знаходяться в середовищі розробки:
...
```
Оновіть site.css, щоб включити клас помилки:
...
```
Нарешті, оновіть частковий вираз _ValidationScriptsPartial, щоб включити необроблені файли в блок розробки, а зв'язані/мінифіковані файли — в блок, що не є блоком розробки:
...
# Підтримка Загального регламенту захисту даних

Підтримка GDPR у Razor Pages відповідає підтримці в MVC-додатках. Почніть з додавання CookiePolicyOptions та змініть файли cookie TempData та Session на essential у Program.cs:
...
app.UseRouting();
```
Додайте наступний оператор using до файлу GlobalUsings.cs:
...
Додайте нове представлення з назвою _CookieConsentPartial.cshtml у каталозі Pages\Shared. Це те саме представлення з MVC-застосунку:  
...
```
Нарешті, додайте частковий перегляд до часткового перегляду _Layout:
...
```
Якщо це налаштовано, під час запуску програми ви побачите банер згоди на використання файлів cookie. Якщо користувач натисне Accept, буде створено файл cookie .AspNet.Content. Під час наступного завантаження сайту банер не відображатиметься.

## Елемент меню для прийняття/відкликання згоди на використання політики використання файлів cookie

Остання зміна в застосунку полягає в додаванні підтримки меню для надання або відкликання згоди. Додайте нову пусту сторінку Razor з назвою Consent.cshtml до головного каталогу Pages. Оновіть PageModel до наступного:
...
```
Сторінка Razor має два обробники HTTP-сторінок. Щоб їх викликати, посилання має використовувати допоміжний тег asp-page-handler.
...
```
Якщо банер відображається (користувач не надав згоди), тоді відобразіть посилання меню для користувача, щоб прийняти політику щодо файлів cookie. Якщо вони надали згоду, тоді покажіть посилання меню для відкликання згоди. Зверніть увагу на помічники тегу asp-page-handler:
...
Цей розділ завершив AutoLot.Web. Все почалося з глибокого занурення в Razor Pages та перегляди сторінок, часткові перегляди, а також шаблони редагування та відображення. Наступний набір тем охоплював клієнтські бібліотеки, зокрема керування бібліотеками в проєкті, а також їх об'єднання в пакети та мініфікацію.
Далі було розглянуто тег-хелпери та створення користувацьких тег-хелпери для проєкту.
Cars Razor Pages були створені разом із власним базовим класом. Було додано компонент перегляду, щоб зробити меню динамічним, адміністративну панель та її сторінки, і, нарешті, розглянуто питання валідації та підтримки GDPR.
