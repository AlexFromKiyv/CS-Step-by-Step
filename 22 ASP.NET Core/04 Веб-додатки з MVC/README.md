...
Щоб відобразити представлення з назвою, яка відрізняється від назви методу дії, передайте назву файлу (без розширення cshtml).Наведений нижче код відобразить представлення MyIndex.cshtml:
...
```
HTML-хелпер DisplayNameFor() відображає назву властивості, якщо тільки властивість не оформлена атрибутом Display(Name='') або DisplayName(''), і в цьому випадку використовується значення відображення.
...
```
Додайте файл Views\Cars\Templates.cshtml
...
```
Шаблон редактора використовує кілька тег-хелперів (asp-for, asp-items, asp-validation-for та asp-validation-summary). Вони будуть розглянуті пізніше в цій главі.
...
Змініть файл Views\Cars\Templates.cshtml
...
Коли ви переглянете sources в інструментах розробника, ви побачите файл site.css (у папці css) та AutoLot.Mvc.Styles.css, у кореневому каталозі, який поєднує CSS з представлень _Layout.cshtml.css та Index.cshtml.css.
...
```
Будь-який код та/або розмітка в блоці @section файлу представлення не буде відображатися за допомогою виклику @RenderBody(), а буде відображатися в місці визначення розділу макета. Наприклад, припустимо, що у вас є представлення з такою реалізацією section в Views\Cars\Templates.cshtml :
...
```
Якщо представлення не має спеціально визначеного макета, воно шукатиме файл _ViewStart.cshtml у тому ж каталозі, що й представлення. Якщо такого макета немає в тому самому каталозі, виконується пошук у кожному каталозі вище поточного розташування, доки не буде знайдено такий макет, використовуючи перший знайдений файл _ViewStart.cshtml (незалежно від того, чи визначено макет). Якщо такого не знайдено, макет не використовується, по суті, відображається лише часткове представлення.
...
```
Тег \<partial\> – це ще один приклад тег-хелпера. Атрибут name – це назва часткового елемента, шлях до якого починається з поточного каталогу представлення (без розширення .cshtml), у цьому випадку це Views\Shared.
...
Останній крок на цьому етапі — вирізати теги \<script\> для файлів JavaScript та вставити їх у частку _JavaScriptFiles. Обов’язково залиште тег RenderSection на місці. Ось фрагмент JavaScriptFiles:
...
## Строго типізовані представлення та моделі представлень
...
```html
<hr/>
The Car named @Model.PetName is a <span style=" color : @Model.Color ">@Model.Color</span> @Model.MakeName
...
Розташування файлів Bootstrap потрібно оновити на ~/lib/bootstrap/css замість ~/lib/boostrap/dist/css, а також додати Font Awesome. 
Оновіть файл _Head.cshtml до наступного.
...
Менеджер бібліотек легко та ефективно встановлює клієнтські бібліотеки у ваш проект. Два додаткові міркування щодо створення вебзастосунків з клієнтськими бібліотеками - це пакетування та мініфікація для покращення продуктивності.
...
Коли файл змінюється, хеш змінюється, і браузер перезавантажує файл, оскільки URL-адреса змінилася. Так само функціонує asp-append-version. Тег-хелпер далі в цьому розділі. 
...
Перш ніж продовжити огляд аспектів інтерфейсу користувача застосунків у стилі MVC, час оновити HomeController для використання атрибутів маршрутизації та додати додаткові контролери для основного застосунку. AutoLot.Mvc має базовий контролер та два похідні контролери.
...
```
Додайте конструктор, який приймає екземпляр IAppLogging\<TController\> та IDataServiceBase\<TEntity\> та призначає їх полям рівня класу:
...
Метод IndexAsync() (і майбутнє представлення) використовується для відображення списку всіх сутностей. Цей список отримується за допомогою методу GetAllAsync() у класі служби даних, а потім передається до представлення:
...
Продовжимо реалізовувати базовий контролер. Метод DetailsAsync() використовується для відображення окремого запису під час виклику з HTTP-GET методом. Метод GetOneEntityAsync якій отримує сутність ми реалізували.  Маршрут контролера розширюється необов'язковим значенням ідентифікатора. Необов'язкові токени маршруту позначені знаком питання в токені та представляють параметри, що допускають значення null, у методі дії. Додайте наступний метод дії:
...
Методи BadRequest() та NotFound() – це два допоміжні методи ControllerBase, розглянуті в початковій главі про ASP.NET Core. Це зручні методи для повернення кодів стану HTTP 400 та 404 клієнту.
...
<a asp-controller="TestBaseCrud" asp-action="Index"> To List</a>
```
Додамов в \Views\TestBaseCrud\Index.cshtml

```html
<h6>Test Details</h6>
<a href="https://localhost:5001/TestBaseCrud/Details/5"> .../Details/5</a>&nbsp;
<a href="https://localhost:5001/TestBaseCrud/Details/"> .../Details/</a>&nbsp;
<a href="https://localhost:5001/TestBaseCrud/Details/215"> .../Details/215</a>&nbsp;
...
```
Для того аби пройшла валідація на стороні клієнта тут додано поле TimeStamp з значенням " ". Коли відбувається запис даних СУБД генерує значення.
...
```html
<h6>Test Create</h6>
<a href="https://localhost:5001/TestBaseCrud/Create"> .../Create</a>
```

### Метод Create Post

Метод дії HTTP post CreateAsync() використовує неявне зв'язування моделі для створення екземпляра сутності типу TEntity зі значень форми:
...
Для тестування нічого додавати не треба.
...
Метод дії HTTP get EditAsync() отримує окрему сутність за ідентифікатором, використовуючи обгортку сервісу, та надсилає її до представлення.
...
```
Маршрут має параметр необов'язковий параметр ідентифікатора, який потім передається в метод. Метод використовує допоміжний метод GetOneEntityAsync() для отримання запису. Якщо entity має значення null шо значить запис неможливо знайти, метод додає повідомлення про помилку до ViewData та повертає представлення без будь-яких даних. В іншому випадку він створює SelectList значень пошуку, додає його до словника ViewData та відтворює представлення.
...
@if (ViewData["Error"] != null)
{
    ...
    <a asp-controller="TestBaseCrud" asp-action="Index">Back</a>
}
else
{
    <h6>@ViewData["Title"]</h6>
...
```html
<h6>Test Edit</h6>
<a href="https://localhost:5001/TestBaseCrud/Edit/1"> .../Edit/1</a>
<a href="https://localhost:5001/TestBaseCrud/Edit/"> .../Edit/</a>
<a href="https://localhost:5001/TestBaseCrud/Edit/303"> .../Edit/303</a>
```
### Метод Edit Post
...
```html
<h6>Test Delete</h6>
<a href="https://localhost:5001/TestBaseCrud/Delete/1"> .../Delete/1</a>
```
...
## Перегляди Admin Area

Оновіть файл _Menu.cshtml, щоб додати посилання на меню для області адміністрування:

```html
                <li class="nav-item">
                    <a class="nav-link text-dark" asp-area="Admin" asp-controller="Makes" asp-action="Index" title="Makes Admin">Makes Admin <i class="fas fa-cog"></i></a>
                </li>
```


Останнім кроком для завершення роботи програми AutoLot.Mvc є додавання решти представлень для області Admin.
Додайте чотири представлення Razor: Index.cshtml, Details.cshtml, Create.cshtml, Delete.cshtml  та Edit.cshtml до каталогу Areas\Admin\Views\Make. Ці представлення мають той самий шаблон, що й представлення Cars, тому вони просто перелічені тут:
...
<form asp-action="Create">
    ...
    <div class="d-flex flex-row mt-3">
        <input type="hidden" asp-for="TimeStamp" value=" " />
...
Допоміжний тег дії форми використовується для кнопок та зображень, щоб змінити дію для форми, яка їх містить. Наприклад, наступна кнопка, додана до форми редагування, призведе до того, що запит на публікацію перейде до кінцевої точки Create контролера:

Приклад в RazorSyntax.cshtml 
...
<form method="post">
    ...
    <button type="submit" asp-controller ="TestBaseCrud" asp-action="Create">Add</button>
</form>
```

Якшо контролер не вказувати буде буде використовуватисть поточний

```html
    <button type="submit" asp-action="Create">Add</button>
```
...
Типи HTML, згенеровані з типів .NET за допомогою тег-хелпера input
...
Крім того, тег-хелпер \<input\> додасть атрибути типу HTML5 на основі анотацій даних. У таблиці наведено деякі з найпоширеніших анотацій та згенеровані атрибути типу HTML5.
...
```
В RazorSyntax.cshtml 

```html
<select asp-for="MakeId" asp-items="@ViewBag.LookupValues"></select>
```

## Тег-хелпер Validation

Допоміжні засоби тегів повідомлення перевірки та підсумку перевірки точно повторюють допоміжні засоби HTML Html.ValidationMessageFor та Html.ValidationSummaryFor. Перший застосовується до HTML span для певної властивості моделі, а другий застосовується до тегу div і представляє всю модель. У зведенні перевірки є варіанти «All», «ModelOnly» (за винятком помилок у властивостях моделі) або «None».
Згадайте тег-хелпери перевірки з EditorTemplate файлу Car.cshtml:
...
Допоміжний тег <environment> зазвичай використовується для умовного завантаження файлів JavaScript та CSS (або будь-якої розмітки) на основі середовища, в якому працює сайт. Відкрийте фрагмент _Head.cshtml та оновіть розмітку до наступного:
...
Тег-хелпер script схожий на \<link\> з налаштуваннями кеш-бастингу та резервного CDN. Атрибут asp-append-version працює так само для скриптів, як і для пов'язаних таблиць стилів.
...
Перший рядок змінює тег з тегу, що використовується в розмітці, на тег anchor. Наступний рядок використовує статичний метод UrlHelper.Action() для генерації маршруту, включаючи параметр маршруту, якщо такий існує. Наступні два встановлюють HREF тегу прив'язки на згенерований маршрут і додають назву класу CSS. В останньому рядку додано текст для відображення та шрифт Font Awesome як текст, який відображається користувачеві.
...
Під час виклику тег-хелпера суфікс TagHelper пропускається, а решта назви класу починається з малої літери. У цьому випадку тег HTML — <item-details>. Значення asp-route-id походить з атрибута item-id на допоміжному механізмі тегів:
...
Хоча тег-хелпери замінили багато HTML-хелперів, все ще існує кілька таких, для яких немає еквівалента. У таблиці перелічені деякі, які досі широко використовуються.
...
Допоміжний метод DisplayFor() відображає об'єкт, визначений виразом. Якщо для відображаного типу існує шаблон відображення, він буде використаний для створення HTML-коду елемента. Наприклад, у перегляді RazorSyntax інформацію про марку автомобіля можна відобразити за допомогою цього коду:
...
Помічники EditorFor() та EditorForModel() функціонують так само, як і відповідні помічники відображення. Різниця полягає в тому, що пошук шаблонів здійснюється в каталозі EditorTemplates, а замість представлення об'єкта лише для читання відображаються HTML input.
...
```
У наступній розмітці використовується ItemCreateTagHelper для створення посилання на метод Create HTTP Get. Нагадуємо, що під час використання кастомних тег-хелперів назва пишеться з малої літери як шашлик. Це означає, що суфікс TagHelper відкидається, а потім кожне слово, що починається у великому регістрі Паскаля, переходить у нижній регістр і розділяється дефісом:
...
Аби краше зрозуміти тему краше згадати як взагалі застосовуються атрібути до класів. Для цього праше перечитати розділ "Атрібути" з глави "Рефлексія, Late Binding, атрібути та динамічні типи".
ASP.NET Core та EF Core містять значну кількість анотацій даних та атрибутів перевірки, які можна застосовувати до моделей (і моделей представлень). Як ви бачили, ці атрибути забезпечують перевірку на стороні клієнта та сервера. Коли ввімкнено перевірку на стороні клієнта, дані перевіряються перед надсиланням запиту на сервер, і разом із тег-хелперами перевірки повідомляють про помилки користувачеві. Серверна перевірка відбувається, якщо JavaScript увімкнено (або бібліотеки jQuery Validation та jQuery Validation Unobstructive не завантажено), виконується серверна перевірка та оновлюється ModelState. Як ви вже бачили, якщо ModelState не валідна, методи дій повертають дані та помилки назад до представлення, що призводить до того ж ефекту, що й під час перевірки на стороні клієнта.
...
```
Далі потрібно додати метод greaterthanzero() до системи валідації jQuery за допомогою функції addMethod() на об'єкті validator. Додайте новий каталог з назвою validations у каталозі wwwroot\js у проекті AutoLot.Mvc. Створіть новий файл з назвою validators.js. Оновіть код до наступного:
...
В налаштуванні атрибутів валідації, ми додамо деякі власні стилі до процесу валідації jQuery. Додайте новий файл з назвою errorFormatting.js у каталозі js\validations проєкту AutoLot.Mvc та оновіть код до наступного:
...
Останньою зміною в застосунку є додавання підтримки меню для надання або відкликання згоди. Додайте до HomeController два наступні методи дій. Кожен метод використовував ITrackingConsentFeature для надання або відкликання згоди, а потім перенаправляв користувача до Index:
...
```
Якщо банер відображається (користувач не надав згоди), тоді відобразіть посилання меню для користувача, щоб прийняти політику щодо файлів cookie. Якщо вони надали згоду, тоді відобразіть посилання меню для відкликання згоди:
...
# Запуск AutoLot.Mvc та AutoLot.Api разом

До цього моменту всі демонстрації використовували рівень доступу до даних служб даних. Щоб перейти на використання служби AutoLot.Api як серверної частини, змініть UseApi у appsettings.Development.json на true. Під час запуску AutoLot.Mvc має бути запущено AutoLot.Api. Це можна зробити за допомогою Visual Studio, командного рядка або їх комбінації.

    Пам’ятайте, що як AutoLot.Mvc, так і AutoLot.Api можна налаштувати на перебудову бази даних щоразу під час запуску. Обов’язково вимкніть хоча б один із них, інакше вони конфліктуватимуть. Щоб пришвидшити налагодження, вимкніть прапорець в обох проектах, коли тестуєте функціональність, яка не змінює жодних даних.

## Використання Visual Studio

Ви можете налаштувати Visual Studio для одночасного запуску кількох проектів. Це досягається клацанням правою кнопкою миші на рішенні в Solution Explorer, вибором пункту Configure Sturtup Progects > Configure Sturtup Progects  та встановленням дій для AutoLot.Api та AutoLot.Mvc на Start.
Коли ви натискаєте F5 (або клацаєте зелену стрілку запуску), обидва проекти запустяться. При цьому виникають деякі тонкощі. По-перше, Visual Studio запам'ятовує останній профіль, який використовувався для запуску програми. Це означає, що якщо ви використовували IIS Express для запуску AutoLot.Api, то одночасний запуск обох призведе до запуску AutoLot.Api за допомогою IIS Express, а порт у налаштуваннях служби буде неправильним. Це досить легко виправити. Або змініть порти у файлі appsettings.Development.json, або запустіть програму за допомогою Kestrel, перш ніж налаштовувати кілька параметрів запуску.
Друга проблема зводиться до часу. Обидва проекти починаються практично одночасно. Якщо AutoLot.Api налаштовано на повторне створення бази даних під час кожного запуску, він не буде готовий до AutoLot.Mvc, коли ViewComponent виконається для створення меню. Швидке оновлення браузера AutoLot.Mvc (після того, як ви побачите SwaggerUI в AutoLot.Api) вирішить цю проблему.

Можна запустити проекти ще простіше. Встановити Configure Sturtup Progects > Current selection. Потім по черзі запустити проекти переходячи з одног в дугий.

## Використання командного рядка

Відкрийте командний рядок у кожному з каталогів проекту та введіть dotnet watch run. Це дозволяє контролювати порядок і час виконання, а також гарантує, що програми виконуватимуться за допомогою Kestrel, а не IIS. Інформацію про налагодження під час запуску з командного рядка див. у розділі "Знайомство з ASP.NET Core".

# Підсумки

У цій главі було завершено розробку програми AutoLot.Mvc. Все почалося з глибокого занурення у представлення, часткові представлення, а також шаблони редактора та відображення. Наступний набір тем охоплював клієнтські бібліотеки, включаючи керування бібліотеками в проекті, а також їх об'єднання та мініфікацію. 
Було розроблено BaseCrudController та CarsController, досліджено Areas та завершено MakesController. Далі було розглянуто допоміжні функції тегів та створення користувацьких допоміжних функцій тегів для проектів.
Було створено представлення Cars та розроблено компонент MenuViewComponent для динамічного меню. Створені перегляди для CRUD. Далі було розроблено два власні атрибути перевірки та додано підтримку GDPR для згоди на використання файлів cookie.
У наступному розділі буде завершено розробку застосунку AutoLot.Web.
