...
```
В цьому прикладі створюється екземпляр класу Exeption з використанням конструктора який заповнює read-only поле. Аби використати об'єкт винятку і поверути в код шо визиває використовується trow.
Отже по-перше винток стврюється коли ви знаете шо обставитни стану обє'кту такі шо далі нема сенсу виконувати код. В данному випадку двигун перегрівся і нема сенсу додавати швидкість. Тобто ми вирішуемо коли відбуваеться виняток а не система наштовхуеться на нього.
В цьому випадку можна було б реалізувати метод без винятків. Як правило винятки створюються при виконані термальних умов(нема мережи, нема файлу, неможливо підключитись до бд), а не для механізму логічного потоку. Рішення про те, що саме виправдовує створення винятку, є проблемою дизайну, з якою ви завжди повинні боротися. Для данного випадку припустимо що перевищення швидкості дійсно виняток а ніж правило.
По-друге зверніть увагу шо виділено блок else. Коли виникає виняток він повертаеться в метод який його викликав (або в блок catch). Тому потреби в else немає.
Ця реалізація вже не робить спроби підвишити швиджкість і видає дамп викликів і рядок де все відбулося але вона закінчує програму з помилкою.

## catch

Якшо ви найшли місце да може виникнути винаток обробити ситуацію можна краще.
