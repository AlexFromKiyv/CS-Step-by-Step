...
Методи розширеня дозволяють додавати нову функціональність до існуючих класів без необхідності створювати підклас. Вони дозволяють це робити і для класів та структур які не дозволяють успадкування.  
...
Використовуючи LINQ, можна стnворювати конструкції безпосередньо в C#, які називають виразами запитів. Вираз запиту можна використовувати для взаємодії з багатьма типами даних. По суті це опис загального підходу до доступу до даних. 
...
Створимо проект LinqOverArray і допоміжний метод в Program.cs
...
```
Атакож визначемо данні з якими будемо працювати.
...
Uncharted 2
```
Таким чином видно що LINQ можна використовувати для радикального спрощеня в вилученні і отримання підмножин набору даних. Замість створення вкладених циклів та складних умов логіки компілятор зробить цю роботу від вашого імені, коли ви створите відповідний запит LINQ. 
...
```
Ми бачимо що змінна результату екземпляр типу OrderedEnumerable\<TElement,TKey\> яка є внутрішнsм абстрактним типом, що міститься в збірці System.Linq.

Зробимо такеж саме дослідження для LINQ у вигляді методів розширення.
```cs
...
Result is type of:SelectIPartitionIterator`2
This type locate:System.Linq
```
...
Нехай у нас в проекті LinqOverCollections є клас Car.cs
...
Для розгляду виразів Linq використаємо наступний контекст в проекті LinqExpressions.
...
Classic Valpo Pizza           Everyone loves pizza!         73
```
При створені логічного виразу використовується змінна яка вказана в from. Для where логічний вираз може бути складнішим але повинен бути валідним. 
```cs
...
Припустимо ми маємо тип (Types.cs) 
```cs
...
```
Таким чином в результаті запиту ми отримуємо коллекцію з необхідним нам типом. Таким чином, залежно від потреб, можна мати вібір як робити проекцію. 

## Змінні в запросах та оператор let.

В межах запиту можан визначити зміну для проміжних обчислень.

Припустимо у нас є класс (Types.cs).
```cs
...
За допомогою LINQ можна отримани дані з декількох джерел (Types.cs).
...
Для методів що об'єднують, виднімають і перетинають набори даних присутня можливість вказувати селектори. Селектори використовують певні властивості об'єктів, шоб визначити дію, яку потрібно виконати.
