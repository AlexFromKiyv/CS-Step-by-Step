...
{
    StationsPresets = [89.3, 105.1, 97.1],
    HasTweeters  =true
};
...
TravelCar trevelCar = new()
{
    ...
    {
        HasTweeters = true,
        StationsPresets = new() { 89.3, 105.1, 97.1 }
    }
};
...
Атрибути простору імен System.Xml.Serialization

|Атрібут|Використання|
|-------|------------|
...
Створемо функцію считтуваня з файлу
...
JsonSerializer містить статичні методи Serialize, які використовуються для перетворення об’єктів .NET (включаючи графи об’єктів) у рядкове представлення загальнодоступних властивостей. Дані представлені як пари ім’я-значення в нотації об’єктів JavaScript. Cтворемо функцію для форматування. 
...
Потрібно проставити атрібути полів для серіалізації.
...
Ви почали цю главу з розгляду використання типів Directory(Info) та File(Info). Ви почали цей розділ з розгляду використання типів Directory(Info) та File(Info). Далі ви розглянули кілька класів, похідних від абстрактного класу Stream. Оскільки типи, похідні від Stream, працюють із необробленим потоком байтів, простір імен System.IO надає численні типи зчитування/запису (наприклад, StreamWriter, StringWriter та BinaryWriter), які спрощують процес. По дорозі ви також ознайомилися з функціональністю, що надає DriveType, навчилися моніторити файли за допомогою типу FileSystemWatcher та побачили, як взаємодіяти з потоками асинхронно.
У цій главі також було ознайомлено вас із темою сервісів серіалізації об'єктів. Як ви бачили, платформа .NET використовує граф об'єктів для врахування повного набору пов'язаних об'єктів, які потрібно зберегти в потоці.
